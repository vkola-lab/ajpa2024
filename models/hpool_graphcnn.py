...
class GraphCNN(nn.Module):
    def __init__(self, num_layers, num_mlp_layers, input_dim, hidden_dim, output_dim, final_dropout, learn_eps, graph_pooling_type, pooling_ratio, neighbor_pooling_type, device):
        ...
        for layer in range(self.num_layers-1):
            ...
            self.pool.append(SAGPool(hidden_dim, ratio=self.pooling_ratio))
    ...
    def __preprocess_neighbors_sumavepool(self, batch_graph):
        ...
        start_idx = [0]
        for i, graph in enumerate(batch_graph):
            start_idx.append(start_idx[i] + len(graph.node_features))
    ...
    def __preprocess_graphpool(self, batch_graph):
        ...
        for i, graph in enumerate(batch_graph):
            ...
            if self.graph_pooling_type == "average":
                elem.extend([1./len(graph.node_features)]*len(graph.node_features))

            else:
            ###sum pooling
                elem.extend([1]*len(graph.node_features))
